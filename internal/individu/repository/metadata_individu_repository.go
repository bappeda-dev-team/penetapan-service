...
func (repo *PenetapanIndividuRepository) InsertMetadata(
	ctx context.Context,
	metadata domain.SyncPenetapanMetadata,
) (int64, error) {

	query := `INSERT INTO
		  sync_penetapan_metadata_individu
...
func (repo *PenetapanIndividuRepository) UpdateMetadataStatus(
	ctx context.Context,
	metadataId int64,
	status string,
	errMessage *string,
	...
	if status == domain.SyncStatusSuccess ||
		...
		query = `
		UPDATE sync_penetapan_metadata_individu
		SET
			status = $1,
			error_message = $2,
			finished_at = NOW(),
