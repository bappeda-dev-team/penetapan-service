...
func (ex *SasaranSyncExecutor) Sync(
	...
	sasarans, err := ex.Client.SyncSasaranPemdaPenetapan(ctx, client.SyncRequest{Tahun: req.Tahun})
	if err != nil {
	...
	for _, sasaran := range sasarans {
		sasaranID, err := ex.Repo.SaveSasaranPemdaPenetapan(
			ctx,
			tx,
			domain.SasaranPemdaPenetapan{
				KodeSasaranPemda: fmt.Sprintf("SAS-PEMDA-%d", sasaran.Id),
				SasaranPemda:     sasaran.SasaranPemda,
				Periode: fmt.Sprintf(
					"%s-%s", sasaran.Periode.TahunAwal, sasaran.Periode.TahunAkhir),
